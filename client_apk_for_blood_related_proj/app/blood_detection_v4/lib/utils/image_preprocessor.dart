...
class ImagePreprocessor {
  static Future<ImageInfoData> preprocess(
    File imageFile, {
    int inputWidth = imageInputWidth,
    int inputHeight = imageInputHeight,
  }) async {
    final bytes = await imageFile.readAsBytes();
    img.Image? image = img.decodeImage(bytes);
    if (image == null) throw Exception("Unable to decode image");
    return _preprocessImage(image, inputWidth, inputHeight);
  }

  static Future<ImageInfoData> preprocessCameraImage(
    CameraImage cameraImage, {
    int inputWidth = liveInputWidth,
    int inputHeight = liveInputHeight,
  }) async {
  ...
  static ImageInfoData _preprocessImage(
    img.Image image,
    int inputWidth,
    int inputHeight,
  ) {
